...
import org.bitcoinj_extra.core.DumpedPrivateKey;
...
import org.bitcoinj_extra.crypto.BIP38PrivateKey;
...
import com.matthewmitchell.bitcoin_extra_wallet.util.Qr;
		...
		public void parse()
		{
			...
			else if (input.startsWith("bitcoin:"))
			{
				try
				{
					...
					if (address != null && !address.getCoinDetails().contains(Constants.NETWORK_PARAMETERS))
	...
	public static PaymentIntent parsePaymentRequest(final byte[] serializedPaymentRequest) throws PaymentProtocolException
	{
		try
		{
			...
			final PaymentIntent paymentIntent = new PaymentIntent(PaymentIntent.Standard.BIP70, pkiName, pkiCaName,
					...
					Arrays.asList((CoinDetails) MainNetParams.get()));
